...
class mask_generator:
    ...
    def _retrieve_box(self, img, seed):
        print("retrieve_box")
        np.random.seed(seed)
    ...
    def _retrieve_random(self, img, seed):
        np.random.seed(seed)
        idx=seed
        ...
        mask[:, ...] = mask_b
        # torch.save(mask, "/gpfs/share/home/2301111469/dps/exp/s0_inp_operator/"+str(idx)+'.pt')
        return mask

    def __call__(self, img, seed):
        if self.mask_type == 'random':
            mask = self._retrieve_random(img, seed)
            return mask
        elif self.mask_type == 'box':
            mask, t, th, w, wl = self._retrieve_box(img, seed)
            return mask
...
class Blurkernel(nn.Module):
    def __init__(self, blur_type='gaussian', kernel_size=31, std=3.0, device=None):
        ...
        self.seq = nn.Sequential(
            ...
            nn.Conv2d(1, 1, self.kernel_size, stride=1, padding=0, bias=False, groups=1)
        )
