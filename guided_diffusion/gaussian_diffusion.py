...
def create_sampler(sampler,
    ...
    print(__SAMPLER__)
    # {'ddpm': <class 'guided_diffusion.gaussian_diffusion.DDPM'>, 'ddim': <class 'guided_diffusion.gaussian_diffusion.DDIM'>}
    print(steps)
...
class GaussianDiffusion:
    ...
    def p_sample_loop(self,
        ...
        ###
        ### p(y|x_0_hat) x_0_hat comes from x_t
        ###
        img = x_start
    ...
    def p_mean_variance(self, model, x, t):
        ...
        if model_output.sample.shape[1] == 2 * x.shape[1]:
            model_output, model_var_values = torch.split(model_output.sample, x.shape[1], dim=1)
        else:
            ...
            model_var_values = model_output.sample

        model_mean, pred_xstart = self.mean_processor.get_mean_and_xstart(x, t, model_output.sample)
